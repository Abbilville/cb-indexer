...
export function ServiceDetailModal({
  ...
  const [reindexEngine, setReindexEngine] = useState<'both' | 'ast' | 'cpg'>('both');
  ...
  const handleReindex = async () => {
    try {
      ...
      showToast(`${reindexEngine.toUpperCase()} re-indexing triggered for ${repo.name}`, 'success');
      onReindexTriggered();
    } catch (err: unknown) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="relative w-full max-w-2xl max-h-[88vh] flex flex-col p-6 bg-gray-900/95 border border-white/10 rounded-2xl shadow-2xl backdrop-blur-xl overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto py-4 space-y-5 pr-1">
          {/* Status & Quick Stats (AST + CPG) */}
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
            <div className="p-2.5 rounded-xl bg-black/40 border border-white/5">
              <div className="text-[10px] text-gray-400 flex items-center gap-1.5 mb-0.5">
            ...
            <div className="p-2.5 rounded-xl bg-black/40 border border-white/5">
              ...
              <div className="text-xs font-semibold">
                {repo.is_cpg_indexed ? (
                  <span className="text-cyan-400">Indexed</span>
                ) : (
                  <span className="text-gray-500">Unindexed</span>
                )}
              </div>
            </div>
        ...
        {/* Footer Actions */}
        <div className="flex items-center justify-between pt-4 border-t border-white/10 shrink-0 gap-3">
          <div className="flex items-center gap-2">
            <div className="flex items-center p-0.5 rounded-lg bg-black/40 border border-white/10 text-[11px] font-mono">
              <button
                type="button"
                onClick={() => setReindexEngine('both')}
                className={`px-2 py-1 rounded text-center transition-all ${
                  reindexEngine === 'both' ? 'bg-blue-600 text-white font-semibold' : 'text-gray-400 hover:text-white'
                }`}
                title="Re-index both AST and CPG"
              >
                Both
              </button>
              <button
                type="button"
                onClick={() => setReindexEngine('ast')}
                className={`px-2 py-1 rounded text-center transition-all ${
                  reindexEngine === 'ast' ? 'bg-purple-600/40 text-purple-200 font-semibold' : 'text-gray-400 hover:text-white'
                }`}
                title="Re-index AST only"
              >
                AST
              </button>
              <button
                type="button"
                onClick={() => setReindexEngine('cpg')}
                className={`px-2 py-1 rounded text-center transition-all ${
                  reindexEngine === 'cpg' ? 'bg-cyan-600/40 text-cyan-200 font-semibold' : 'text-gray-400 hover:text-white'
                }`}
                title="Re-index CPG only"
              >
                CPG
              </button>
            </div>
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-gray-200 bg-white/10 hover:bg-white/15 border border-white/10 rounded-xl transition-all disabled:opacity-50"
            >
              <RefreshCw className={`w-3.5 h-3.5 text-blue-400 ${isReindexing ? 'animate-spin' : ''}`} />
              <span>{isReindexing ? 'Triggering...' : `Re-index (${reindexEngine.toUpperCase()})`}</span>
            </button>
          </div>
          <button
            onClick={onClose}
            className="px-4 py-1.5 text-xs font-semibold text-white bg-white/10 hover:bg-white/15 rounded-xl transition-all"
          >
            Close
          </button>
        </div>
